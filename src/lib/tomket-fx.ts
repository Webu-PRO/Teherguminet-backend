...
export const resolveTomketPricing = async (
  ...
  if (!fx || isTomketFxStale(fx)) {
    const { missing } = resolveTomketPricingConfig({ eurHufRate: undefined })
    return {
      missing: [
  ...
  const markupPercent = resolveTomketFxMarkupPercent()
  const rate = applyTomketFxMarkup(fx.rate, markupPercent)
  const { config, missing } = resolveTomketPricingConfig({ eurHufRate: rate })
