...
export type OwnDeliveryFulfillmentCreatedEmailProps = {
...
export const OwnDeliveryFulfillmentCreatedEmail = ({
  order,
  ...
  const languageBlocks: Record<LanguageCode, LanguageBlock> = {
    hu: {
      code: "hu",
      preview: (id) => `Saját szállítás előkészítve: ${id}`,
      heading: "Saját szállítás előkészítve",
      intro: (name, id) => (
        <>
          Kedves {name}! A <strong style={styles.strong}>{id}</strong>{" "}
          rendelésed saját szállításra előkészítve.
        </>
      ),
      promiseLines: [
        "A szállítás hamarosan elindul, és a csomag rövid időn belül megérkezik.",
        "Amennyiben kérdésed van, ügyfélszolgálatunk készséggel segít.",
      ],
      details: {
    ...
    sk: {
      code: "sk",
      preview: (id) => `Vlastné doručenie pripravené: ${id}`,
      heading: "Vlastné doručenie pripravené",
      intro: (name, id) => (
        <>
          Dobrý deň {name}! Vaša objednávka{" "}
          <strong style={styles.strong}>{id}</strong> je pripravená na vlastné
          doručenie.
        </>
      ),
      promiseLines: [
        "Doručenie sa čoskoro začne a balík vám dorazí v krátkom čase.",
...
export const mockOwnDeliveryFulfillmentCreated: OwnDeliveryFulfillmentCreatedEmailProps =
  {
    order: {
