...
export const OwnDeliveryShippedEmail = ({
  ...
  const languageBlocks: Record<LanguageCode, LanguageBlock> = {
    hu: {
      ...
      heading: "Elindult a szállítás",
      intro: (name, id) => (
        <>
          Kedves {name}! A <strong style={styles.strong}>{id}</strong>{" "}
          rendelésed szállítása elindult.
        </>
      ),
      promiseLines: [
    ...
    sk: {
      ...
      heading: "Doručenie je na ceste",
      intro: (name, id) => (
        <>
          Dobrý deň {name}! Doručenie vašej objednávky{" "}
          <strong style={styles.strong}>{id}</strong> sa začalo.
        </>
      ),
      promiseLines: [
        "Balík vám čoskoro dorazí.",
