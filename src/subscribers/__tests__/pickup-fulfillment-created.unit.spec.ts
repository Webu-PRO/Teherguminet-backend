...
describe("pickup-fulfillment-created subscriber", () => {
  it("uses fulfillment_id from order.fulfillment_created payload", async () => {
    const graph = jest
      .fn<QueryGraphFn>()
      .mockImplementation(async (input: any) => {
        if (
          input?.entity === "fulfillment" &&
          input?.filters?.id === "ful_61"
        ) {
          return {
            data: [
              {
                id: "ful_61",
                created_at: "2026-03-27T15:39:00.000Z",
                metadata: {},
    ...
    expect(updateOrders).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        metadata: expect.objectContaining({
          ...
          pickup_ready_last_fulfillment_id: "ful_61",
